...
`GET /api/v1/auth/session` (with the temporary `POST` and `/api/auth` aliases) verifies a revocation-aware Firebase ID token and uses only its UID for lookup. The response envelope contains `uid`, `email`, `displayName`, canonical `roles`, `disabled`, `onboardingStatus`, and memberships (`organizationId`, canonical `roles`, and lifecycle status).
...
During the membership migration, a legacy global role may still establish the coarse application role without an active membership. Global `super_admin` is intentionally global and therefore does not require one. Parent, mentor, and observer requests may authenticate under a legacy global role, but organization-owned records remain unavailable unless an active membership supplies the organization scope and the feature's relationship or assignment check also succeeds. Login never creates a membership implicitly.

A first session request idempotently creates a missing `users/{uid}` identity profile with server timestamps and no role. It returns only the deterministic onboarding states `complete`, `role_required`, `pending`, `disabled`, or `session_error`. Pending membership and incomplete child context map to `pending`; suspended membership and disabled identity/profile map to `disabled` with no roles. Expired and malformed membership records fail closed and block legacy-role fallback. Public sign-in cannot select mentor, observer, admin, or super_admin (and does not implicitly select parent).
