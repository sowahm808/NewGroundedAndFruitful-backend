...
export interface LegacyAdministratorBootstrapInput {
  uid: string;
  name: string;
  slug: string;
  timezone: string;
...
export interface LegacyAdministratorBootstrapReport {
  uid: string;
  organizationId: string;
  membershipId: string;
  outcome: "created" | "would_create";
  claims: "refresh_required" | "synchronized" | "not_attempted";
}

/**
 * Trusted CLI boundary for the otherwise circular first-tenant migration.
 * Organization, administrator membership, migration marker, and domain audit
 * records are committed together; Firebase claims are only a cache and are
 * reconciled after that authoritative transaction.
 */
export async function bootstrapLegacyAdministrator(
  auth: Auth,
  db: Firestore,
  raw: LegacyAdministratorBootstrapInput,
): Promise<LegacyAdministratorBootstrapReport> {
  const schema = z
    .object({
  ...
  if (!parsed.success) throw new ValidationError("Invalid bootstrap input.");
  const input = parsed.data;
  if (!validTimezone(input.timezone))
    throw new ValidationError("ORGANIZATION_TIMEZONE_INVALID");
  ...
  if (authUser.disabled) throw new ConflictError("ONBOARDING_NOT_ELIGIBLE");

  const organizationId = `org-${safeId(input.slug).slice(0, 24)}`;
  const membershipId = `${organizationId}_${input.uid}`;
  const result = await db.runTransaction(async (tx) => {
    const [profile, memberships, slugMatches, nameMatches, consumed] =
      await Promise.all([
        tx.get(db.doc(`users/${input.uid}`)),
        tx.get(db.collection("memberships").where("userId", "==", input.uid)),
        tx.get(db.collection("organizations").where("slug", "==", input.slug)),
        tx.get(db.collection("organizations").where("name", "==", input.name)),
        tx.get(db.doc(`migrationRecords/legacy-bootstrap-${input.uid}`)),
      ]);
    ...
    if (
      ...
      !legacyRoles.includes("super_admin")
    )
      throw new ConflictError("ONBOARDING_NOT_ELIGIBLE");
    if (
      consumed.exists ||
      memberships.docs.some((doc) => doc.get("status") === "active")
    )
      throw new ConflictError("ONBOARDING_ALREADY_COMPLETED");
    if (!slugMatches.empty)
      throw new ConflictError("ORGANIZATION_SLUG_CONFLICT");
    if (!nameMatches.empty)
      throw new ConflictError("ORGANIZATION_NAME_CONFLICT");
    if (input.dryRun) return "would_create" as const;
    ...
    tx.create(db.doc(`migrationRecords/legacy-bootstrap-${input.uid}`), {
      type: "legacy_authorization_bootstrap",
      uid: input.uid,
      organizationId,
      membershipId,
      status: "complete",
    ...
    for (const event of [
      "organization.bootstrap_started",
      "organization.created",
      "membership.created",
      "authorization.legacy_migrated",
    ...
    tx.update(profile.ref, {
      legacyAuthorizationMigratedAt: now,
      legacyAuthorizationMigrationId: `legacy-bootstrap-${input.uid}`,
      updatedAt: now,
      updatedBy: input.actor,
    });
...
export async function bootstrapOrganization(
  ...
  if (!parsed.success)
    throw new ValidationError(
      "Invalid organization bootstrap input or IANA timezone.",
    );
  ...
  if (input.environment === "production" && !input.confirmed)
    throw new ValidationError(
      "Production bootstrap requires explicit confirmation.",
    );
  const id = `org-${safeId(input.name, input.timezone).slice(0, 24)}`;
  return db.runTransaction(async (tx) => {
    const all = await tx.get(db.collection("organizations"));
    const exact = all.docs.filter(
      (doc) =>
        doc.get("name") === input.name &&
        doc.get("timezone") === input.timezone,
    );
    if (exact.length > 1 || (exact.length === 1 && all.size > 1))
      throw new ConflictError(
        "Ambiguous duplicate organizations require repair.",
      );
    if (exact.length === 1) {
      if (
        exact[0]!.get("status") !== "active" ||
        exact[0]!.get("version") !== VERSION
      )
        throw new ConflictError(
          "Matching organization is not an exact active version match.",
        );
      return { organizationId: exact[0]!.id, outcome: "existing" };
    }
    if (!all.empty)
      throw new ConflictError(
        "An organization already exists; initial bootstrap is refused.",
      );
    ...
    tx.create(
      db.doc(`auditLogs/bootstrap-${safeId(input.name, input.timezone)}`),
      {
        event: "ORGANIZATION_BOOTSTRAPPED",
        organizationId: id,
        version: VERSION,
        actor: input.actor,
        createdAt: now,
...
export interface ProvisionChildReport {
  ...
  changes: string[];
  claims: "synchronized" | "retry_required" | "not_attempted";
}
...
export async function provisionChild(
  auth: Auth,
  db: Firestore,
  raw: ProvisionChildInput,
): Promise<ProvisionChildReport> {
  const parsed = z
    .object({
      uid: z.string().trim().min(1).max(128),
    ...
    .safeParse(raw);
  if (!parsed.success)
    throw new ValidationError("Invalid child provisioning input.");
  const input = parsed.data;
  if (input.environment === "production" && !input.confirmed)
    throw new ValidationError(
      "Production provisioning requires explicit confirmation.",
    );
  const authUser = await auth.getUser(input.uid).catch((error: unknown) => {
    if (firebaseCode(error) === "auth/user-not-found")
      throw new NotFoundError("Firebase user not found.");
  ...
  if (authUser.uid !== input.uid)
    throw new ConflictError("Firebase identity mismatch.");
  if (input.parentUid)
    await verifyParent(auth, db, input.parentUid, input.organizationId);
  ...
  const result = await db.runTransaction(async (tx) => {
    const [org, user, memberships, mapped, deterministicParticipant] =
      await Promise.all([
        tx.get(db.doc(`organizations/${input.organizationId}`)),
        tx.get(db.doc(`users/${input.uid}`)),
        tx.get(db.collection("memberships").where("userId", "==", input.uid)),
        tx.get(
          db.collection("participants").where("firebaseUid", "==", input.uid),
        ),
    ...
    if (memberships.size > 1)
      throw new ConflictError("Duplicate memberships require repair.");
    ...
    if (membership && membership.get("status") !== "active")
      throw new ConflictError(
        "Existing child membership is not active and requires repair.",
      );
    if (
      membership &&
      !normalizeRoles(
        membership.get("roles") ?? membership.get("role"),
      ).roles.includes("child")
    )
      throw new ConflictError("Existing membership is not a child membership.");
    if (mapped.size > 1)
      throw new ConflictError("Duplicate participant mappings require repair.");
    const uidParticipant = mapped.docs[0];
    if (
      uidParticipant &&
      uidParticipant.get("organizationId") !== input.organizationId
    )
      throw new ConflictError("Participant belongs to another organization.");
    const participant =
      uidParticipant ??
      (deterministicParticipant.exists ? deterministicParticipant : undefined);
    if (
      participant &&
    ...
    if (
      participant &&
      participant.get("firebaseUid") &&
      participant.get("firebaseUid") !== input.uid
    )
      throw new ConflictError("Participant is mapped to another identity.");
    if (participant && participant.get("status") !== "active")
      throw new ConflictError(
        "Existing participant is not active and requires repair.",
      );
    ...
    if (!user.exists) changes.push("user_created");
      ...
      !normalizeRoles(user.get("roles") ?? user.get("role")).roles.includes(
        "child",
      )
    )
      changes.push("user_role_added");
    if (!membership) changes.push("membership_created");
    if (!participant) changes.push("participant_created");
    else if (!participant.get("firebaseUid"))
      changes.push("participant_uid_repaired");
    if (input.dryRun) return { targetMembership, targetParticipant };
    const now = FieldValue.serverTimestamp();
    const existingRoles = user.exists
    ...
    tx.set(
      db.doc(`users/${input.uid}`),
      {
        ...
        roles: [...new Set([...existingRoles, "child"])],
        version: VERSION,
        updatedAt: now,
        updatedBy: input.actor,
    ...
    if (input.parentUid) {
      const link = db.doc(
        `parentChildLinks/${safeId(input.organizationId, input.parentUid, targetParticipant.id)}`,
      );
      tx.set(
        link,
        {
    ...
    tx.set(
      db.doc(
        `auditLogs/child-provision-${safeId(input.organizationId, input.uid)}`,
      ),
      {
        event: "CHILD_PROVISIONED",
        targetUid: input.uid,
        organizationId: input.organizationId,
  ...
  function report(
    outcome: ProvisionChildReport["outcome"],
    claimState: ProvisionChildReport["claims"],
    membership: DocumentReference,
    participant: DocumentReference,
  ): ProvisionChildReport {
    return {
      uid: input.uid,
      organizationId: input.organizationId,
...
async function verifyParent(
  ...
  await auth.getUser(uid).catch(() => {
    throw new NotFoundError("Parent Firebase user not found.");
  });
  const memberships = await db
    .collection("memberships")
    .where("userId", "==", uid)
    .get();
  const authorized = memberships.docs.filter(
    (doc) =>
      doc.get("organizationId") === organizationId &&
      doc.get("status") === "active" &&
      normalizeRoles(doc.get("roles") ?? doc.get("role")).roles.includes(
        "parent",
      ),
  );
  if (authorized.length !== 1)
    throw new ConflictError(
      "Exactly one active parent membership is required.",
    );
}
