...
export const permissions = [
  ...
  "team.composite.read",
  "child.linked.read",
  "observation.linked.create",
  "observation.own.read",
  "family.linked.read",
  "family.linked.complete",
  "support.linked.create",
  "support.own.read",
  "report.linked.read",
  "team.assigned.read",
  "project.assigned.guide",
  "reading.assigned.read",
  "encouragement.assigned.manage",
  "observation.granted.create",
  "observation.granted.read-own",
  "program.read",
  "program.configure",
  "content.manage",
  "quarter.manage",
...
): membership is ActiveMembership => membership.status === "active";
...
export class AuthorizationPolicy {
  ...
  async authorize(
    ...
    if (memberships.length === 0 && !platformOperator)
    ...
    const eligibleRoles = platformOperator
      ? (["platform_super_admin"] as const).filter((role) =>
          rolePermissions[role].includes(permission),
        )
      : [];
    if (eligible.length === 0 && eligibleRoles.length === 0)
      throw new AuthorizationError();
    if (
      programPermissions.has(permission) &&
      resource.programId &&
      !platformOperator &&
      !eligible.some(
        (membership) =>
          !membership.programIds ||
    ...
    let related = true;
    if (childPermissions.has(permission))
      related =
        Boolean(resource.participantId) &&
        (await this.relationships.ownsParticipant(context.actorUid, resource));
    else if (parentPermissions.has(permission))
      related =
        Boolean(resource.participantId) &&
        (await this.relationships.hasParentLink(context.actorUid, resource));
    else if (mentorPermissions.has(permission))
      related =
        Boolean(resource.teamId) &&
        (await this.relationships.hasMentorAssignment(
          context.actorUid,
          resource,
        ));
    else if (observerPermissions.has(permission))
      related =
        Boolean(resource.participantId) &&
        (await this.relationships.hasObserverGrant(
          context.actorUid,
          resource,
          this.clock(),
        ));
...
export class FirestoreRelationshipReader implements RelationshipReader {
  ...
  async ownsParticipant(uid: string, r: ResourceScope) {
    ...
    return (
      doc.exists &&
      doc.get("firebaseUid") === uid &&
      doc.get("organizationId") === r.organizationId &&
      doc.get("status") !== "archived"
    );
  }
  async hasParentLink(uid: string, r: ResourceScope) {
    ...
    return snap.docs.some(
      (d) =>
        d.get("revokedAt") == null &&
        d.get("organizationId") === r.organizationId,
    );
  }
  ...
  async hasObserverGrant(uid: string, r: ResourceScope, now: Date) {
    ...
    return snap.docs.some(
      (d) =>
        d.get("revokedAt") == null &&
        (!date(d.get("expiresAt")) || date(d.get("expiresAt"))! > now),
    );
  }
}
